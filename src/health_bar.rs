...
fn spawn_health_bars(
    ...
    for (entity, mut health_bar) in query.iter_mut() {
        ...
        let health_bar_background_entity = commands.spawn(MaterialMesh2dBundle {
            mesh: Mesh2dHandle(meshes.add(Rectangle::new(health_bar.width, health_bar.height))),
            material: materials.add(Color::BLACK),
            transform: Transform::from_xyz(health_bar.offset.x, health_bar.offset.y, HEALTH_BAR_BACKGROUND_Z),
            ..default()
        }).id();

        let health_bar_foreground_entity = commands.spawn(MaterialMesh2dBundle {
            mesh: Mesh2dHandle(meshes.add(Rectangle::new(health_bar.width, health_bar.height))),
        ...
        commands.entity(entity).push_children(&[health_bar_background_entity]);
        commands.entity(health_bar_background_entity).push_children(&[health_bar_foreground_entity]);
